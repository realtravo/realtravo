    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-24 pt-6">
      <main className="container px-4 max-w-6xl mx-auto">
        ...
        <div className="relative w-full h-[45vh] md:h-[60vh] bg-slate-900 overflow-hidden rounded-[32px] shadow-xl mb-8">
          {/* Action Buttons on top corners of image */}
          <div className="absolute top-4 left-4 right-4 z-50 flex justify-between items-center">
            ...
            <Button 
              ...
              className={`rounded-full w-10 h-10 p-0 border-none shadow-lg backdrop-blur-md ${
                isSaved ? "bg-red-500" : "bg-black/30 text-white"
              }`}
            >
          ...
          <div className="absolute bottom-6 left-6 z-40 max-w-[80%] pointer-events-none">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[1.7fr,1fr] gap-6">
          <div className="flex flex-col gap-6">
            ...
            <section className="bg-white rounded-[28px] p-7 shadow-sm border border-slate-100">
              ...
              <p className="text-slate-500 text-sm leading-relaxed whitespace-pre-line lowercase">
            ...
            {(event.opening_hours || event.days_opened?.length > 0) && (
              <section className="bg-white rounded-[28px] p-7 shadow-sm border border-slate-100">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-black uppercase tracking-tight text-[#008080]">Event Hours</h2>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl">
                    <span className="text-[10px] font-black uppercase text-slate-400">Hours</span>
                    <span className="text-sm font-black text-slate-700 lowercase">
                      {event.opening_hours ? `${event.opening_hours} - ${event.closing_hours}` : "none"}
                    </span>
                  </div>
                </div>
              </section>
                ...
                <div className="flex flex-wrap gap-2">
                  {event.activities.map((act: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 bg-[#F0E68C]/20 px-4 py-2.5 rounded-2xl border border-[#F0E68C]/50">
                      ...
                      <span className="text-[11px] font-black text-[#857F3E] lowercase tracking-wide">
                        {act.name}
                      </span>
                    </div>
                  ))}
          ...
          <div className="lg:sticky lg:top-6 h-fit space-y-6">
            <div className="bg-white rounded-[32px] p-8 shadow-2xl border border-slate-100">
              ...
              <Button 
                ...
                disabled={!canBook}
                className="w-full py-8 rounded-2xl text-md font-black uppercase tracking-widest text-white shadow-xl border-none mb-6"
                style={{ 
                    background: !canBook ? "#cbd5e1" : `linear-gradient(135deg, ${COLORS.CORAL_LIGHT} 0%, ${COLORS.CORAL} 100%)` 
                }}
              >
                {isSoldOut ? "Sold Out" : isExpired ? "Expired" : "Reserve Spot"}
              </Button>
...
const UtilityButton = ({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick: () => void }) => (
  <Button variant="ghost" onClick={onClick} className="flex-col h-auto py-3 bg-[#F0E68C]/10 text-[#857F3E] rounded-2xl border border-[#F0E68C]/20 flex-1">
