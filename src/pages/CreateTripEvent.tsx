...
const CreateTripEvent = () => {
  ...
  // Automatically disable flexible dates if "Event" is selected
  useEffect(() => {
  ...
  const handleNext = () => {
    ...
    if (validateStep(currentStep)) {
      ...
      toast({ title: "Missing Details", description: "All fields marked in red are required.", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-24">
      ...
      <main className="container px-4 py-8 max-w-4xl mx-auto">
        ...
        {/* Step 2: Experience Details & Date Constraint */}
        {currentStep === 2 && (
          <Card className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 space-y-6">
            <div className="space-y-2">
              ...
              <Input className={`rounded-xl h-12 font-bold ${getErrorClass(formData.name)}`} value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Football Finals" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input className={`rounded-xl h-12 font-bold ${getErrorClass(formData.place)}`} value={formData.place} onChange={(e) => setFormData({...formData, place: e.target.value})} placeholder="e.g. Nairobi" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input className={`rounded-xl h-12 font-bold ${getErrorClass(formData.location)}`} value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} placeholder="e.g. Stadium Gate 4" />
            </div>
            ...
            <div className="space-y-4 pt-4 border-t">
              {formData.type === 'trip' ? (
                ...
                <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
                  <p className="text-[10px] font-black uppercase text-amber-600">Events require a fixed date selection.</p>
                </div>
              )}

              {(!formData.is_custom_date || formData.type === 'event') && (
                <div className="space-y-2">
                  <Label className={`text-[10px] font-black uppercase ${attemptedNext && !formData.date ? 'text-red-500' : ''}`}>Select Date *</Label>
        ...
        {/* Step 4: Contact, Location & Images */}
        {currentStep === 4 && (
          <Card className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
            ...
            <div className="pt-6 border-t border-slate-100">
              <h3 className={`text-xs font-black uppercase mb-4 ${attemptedNext && galleryImages.length === 0 ? 'text-red-500' : 'text-[#008080]'}`}>Gallery (Max 5) *</h3>
        ...
        {currentStep === 5 && (
          <Card className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100">
            ...
            {attemptedNext && (formData.is_custom_date || formData.type === 'event') && (!formData.opening_hours || !Object.values(workingDays).some(v => v)) && (
              <p className="text-red-500 text-[10px] mt-4 font-bold uppercase text-center italic">Operating hours and at least one day are required.</p>
            )}
          </Card>
        )}
...
export default CreateTripEvent;1