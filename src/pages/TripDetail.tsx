...
import { 
  MapPin, Share2, ArrowLeft, Heart, Copy, Clock, Phone, Mail
} from "lucide-react";
...
const TripDetail = () => {
  ...
  const fetchTrip = async () => {
    if (!id) return;
    try {
      // Selecting "*" ensures we get phone and email columns
    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-24">
      ...
      <div className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 px-4 py-3 flex justify-between items-center bg-white shadow-sm border-b ${scrolled ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="flex items-center gap-4">
          <Button onClick={() => navigate(-1)} variant="ghost" className="rounded-full"><ArrowLeft /></Button>
      ...
      <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
        ...
        <div>
          ...
          <p className="text-xs font-bold text-slate-700">
            {trip.phone ? trip.phone : "Not Available"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
        ...
        <div>
          ...
          <p className="text-xs font-bold text-slate-700 truncate max-w-[180px]">
            {trip.email ? trip.email : "Not Available"}
          </p>
        </div>
      </div>
    </div>
