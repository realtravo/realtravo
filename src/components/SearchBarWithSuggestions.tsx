...
export const SearchBarWithSuggestions = ({ value, onChange, onSubmit, onFocus, onBlur }: SearchBarProps) => {
  ...
  const handleFinalSubmit = () => {
    saveToHistory(value);
    setShowSuggestions(false); // Close dropdown on search
    onSubmit();
  };
              ...
              {mostPopular.length > 0 && (
                <div className="mb-4">
                  ...
                  {mostPopular.map((item) => (
                    <button key={item.id} onClick={() => { navigate(`/${item.type}/${item.id}`); setShowSuggestions(false); }} className="w-full p-3 flex gap-4 hover:bg-slate-50 rounded-[24px] text-left transition-all">
              ...
              {searchHistory.length > 0 && (
                <div className="px-5 mb-4">
                  <div className="flex justify-between items-center mb-2 text-[10px] font-black text-slate-400 uppercase">
                    ...
                    <button onClick={() => { setSearchHistory([]); localStorage.removeItem(SEARCH_HISTORY_KEY); }} className="text-[#FF7F50]">Clear</button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {searchHistory.map((h, i) => (
                      <Badge key={i} onClick={() => { onChange(h); onSubmit(); setShowSuggestions(false); }} className="cursor-pointer bg-slate-50 hover:bg-[#008080]/10 text-slate-600 border-none py-2 px-4 rounded-xl text-xs font-bold transition-all">{h}</Badge>
                    ))}
                  </div>
                </div>
              )}
              ...
              {isSearching ? (
                ...
                <>
                  ...
                  {suggestions.map((result) => (
                    <button key={result.id} onClick={() => { navigate(`/${result.type}/${result.id}`); setShowSuggestions(false); }} className="w-full p-3 flex gap-4 hover:bg-slate-50 rounded-[24px] text-left group transition-all">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-md flex-shrink-0 group-hover:scale-105 transition-transform duration-500">
                        <img src={result.image_url || "/placeholder.svg"} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 flex flex-col justify-center min-w-0">
                        ...
                        <div className="flex items-center gap-1 text-slate-400 text-[10px] font-bold uppercase truncate"><MapPin className="h-3 w-3" /> {result.location || result.country}</div>
                      </div>
                    </button>
                  ))}
                </>
              ) : hasSearched && (
                <div className="p-10 text-center"><p className="text-slate-400 text-xs font-bold uppercase mb-1">Not Available</p><p className="text-slate-300 text-[10px]">No results found for "{value}"</p></div>
              )}
            </div>
          )}
        </div>
